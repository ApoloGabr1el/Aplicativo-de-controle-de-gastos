...
class tela_despesa(ctk.CTkFrame):
    def __init__(self, master, app):
        super().__init__(master, fg_color="transparent")
        self.app = app

        self.label_titulo = ctk.CTkLabel(
            self, text="Nova Despesa", font=ctk.CTkFont(size=22, weight="bold")
        )
        self.label_titulo.pack(pady=(40, 20))

        self.label_descricao = ctk.CTkLabel(self, text="Descrição:")
        self.label_descricao.pack(pady=(5, 0))
        self.entry_descricao = ctk.CTkEntry(self, width=280, placeholder_text="Ex: Mercado do mês")
        self.entry_descricao.pack()

        self.label_valor = ctk.CTkLabel(self, text="Valor (R$):")
        self.label_valor.pack(pady=(15, 0))
        self.entry_valor = ctk.CTkEntry(self, width=280, placeholder_text="Ex: 150.90")
        ...
        self.botao_salvar = ctk.CTkButton(self, text="Salvar Despesa", command=self.salvar)
    ...
    def salvar(self):
        ...
        sucesso, mensagem = registrar_movimentacao(
            id_usuario, "despesa", descricao, valor, categoria, data
        )
